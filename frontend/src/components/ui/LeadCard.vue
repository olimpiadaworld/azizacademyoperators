<template>
  <article class="lead-card lead-card--operator glass" :class="{ 'lead-card--busy': busy || reminderBusy }">
    ...
    <div v-else class="lead-card__hint lead-card__hint--soft">
      Atkaz, Maslahat, O'qiydi va Ko'tarmadi tugmalari bosilganda shu card ichida izoh kiritish oynasi chiqadi.
    </div>
    ...
    <div class="lead-card__actions lead-card__actions--operator">
      ...
      <button class="status-action not-answered" :disabled="busy || reminderBusy" @click="emitChange('not_answered')" title="Ko'tarmadi">
        <span></span>
        <small>Ko'tarmadi</small>
      </button>
    </div>
  </article>
</template>
