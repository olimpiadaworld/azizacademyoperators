...
class AppUser(models.Model):
    ROLE_CHOICES = (
        ...
        ('boss', 'Boss'),
        ('operator', 'Operator'),
        ('filial_rahbari', 'Menenjer'),
        ('director', 'Bosh direktor'),
        ('director_deputy', 'Bosh direktor orinbosari'),
    )
...
class LeadVisitDecision(models.Model):
    ...
    admin_note = models.CharField(max_length=500, default='', blank=True)
    ...
    payment_not_done = models.BooleanField(default=False)
    payment_not_done_at = models.DateTimeField(null=True, blank=True)
    payment_not_done_by = models.ForeignKey(AppUser, null=True, blank=True, on_delete=models.SET_NULL, db_column='payment_not_done_by_id', related_name='payment_not_done_decisions')
    left_without_payment = models.BooleanField(default=False)
    left_without_payment_at = models.DateTimeField(null=True, blank=True)
    left_without_payment_by = models.ForeignKey(AppUser, null=True, blank=True, on_delete=models.SET_NULL, db_column='left_without_payment_by_id', related_name='left_without_payment_decisions')
    created_at = models.DateTimeField(default=timezone.now)
    updated_at = models.DateTimeField(default=timezone.now)
